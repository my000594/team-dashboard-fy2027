...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "$tempDir=Join-Path $env:TEMP 'notion_info_tmp';" ^
  ...
  "    if($header -match '開始日' -and $header -match '終了日' -and $header -match '種別'){Write-Host \"  Match: $($zip.Name)\" -ForegroundColor Green;$foundCsv=$csv.FullName;break}" ^
  "  };if($foundCsv){break}" ^
  "};" ^
  "if(-not $foundCsv){Write-Host '[ERROR] No info CSV found.' -ForegroundColor Red;Remove-Item $tempDir -Recurse -Force;exit 1};" ^
  "Copy-Item -Path $foundCsv -Destination (Join-Path $dataDir 'info.csv') -Force;" ^
  "(Get-Item (Join-Path $dataDir 'info.csv')).LastWriteTime=Get-Date;" ^
  "Remove-Item $tempDir -Recurse -Force;" ^
  "Write-Host '';Write-Host '================================================';" ^
  "Write-Host ' Done! info.csv updated.';" ^
