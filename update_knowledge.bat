...
echo ================================================
echo  Knowledge CSV Update Tool
echo ================================================
echo.
...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "$tempDir=Join-Path $env:TEMP 'notion_knowledge_tmp';" ^
  ...
  "    if($header -match '種別' -and $header -match 'カテゴリ' -and $header -match 'タグ'){Write-Host \"  Match: $($zip.Name)\" -ForegroundColor Green;$foundCsv=$csv.FullName;break}" ^
  "  };if($foundCsv){break}" ^
  "};" ^
  "if(-not $foundCsv){Write-Host '[ERROR] No knowledge CSV found.' -ForegroundColor Red;Remove-Item $tempDir -Recurse -Force;exit 1};" ^
  "Copy-Item -Path $foundCsv -Destination (Join-Path $dataDir 'knowledge.csv') -Force;" ^
  "(Get-Item (Join-Path $dataDir 'knowledge.csv')).LastWriteTime=Get-Date;" ^
  "Remove-Item $tempDir -Recurse -Force;" ^
  "Write-Host '';Write-Host '================================================';" ^
  "Write-Host ' Done! knowledge.csv updated.';" ^
