...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "$tempDir=Join-Path $env:TEMP 'notion_sales_tmp';" ^
  ...
  "    if($header -match '予算' -and $header -match '実績' -and $header -match '顧客'){Write-Host \"  Match: $($zip.Name)\" -ForegroundColor Green;$foundCsv=$csv.FullName;break}" ^
  "  };if($foundCsv){break}" ^
  "};" ^
  "if(-not $foundCsv){Write-Host '[ERROR] No sales CSV found.' -ForegroundColor Red;Remove-Item $tempDir -Recurse -Force;exit 1};" ^
  "Copy-Item -Path $foundCsv -Destination (Join-Path $dataDir 'sales.csv') -Force;" ^
  "(Get-Item (Join-Path $dataDir 'sales.csv')).LastWriteTime=Get-Date;" ^
  "Remove-Item $tempDir -Recurse -Force;" ^
  "Write-Host '';Write-Host '================================================';" ^
  "Write-Host ' Done! sales.csv updated.';" ^
