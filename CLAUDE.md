...
- 画像はNotionのファイルプロパティではなく**本文テキスト中に`![説明](URL)`と直接書く**方式（2026-08-23追加）。他の添付ファイル系データ（meeting_plan.csv・org_chart.csv・chronicle.csv等）と同じ理由で、Notion直アップロードの画像URLは期限付きプリサインURLになるため、Google Drive等の外部ストレージの恒久リンクを貼る運用にすること
