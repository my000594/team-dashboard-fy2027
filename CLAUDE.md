...
- 評価レベル（1〜5）の基準はページ内に静的なテキストで表示（Lv.1:知っている／Lv.2:触ったことがある／Lv.3:業務で使える／Lv.4:得意／Lv.5:エキスパート。資格はLv.5＝取得済みを表し、保有資格の詳細は各スキルの備考に記載）。基準文言はCSVではなくskill.html内にハードコーディングしているため、基準を変更する場合はskill.html本体を編集すること
...
- 「備考」列（保有資格の内訳など）はマトリクスのドットのホバー（title属性）に加えて、ドットの下に小さいテキストとして常時表示する。ホバーのみだと見落とされやすいため。スキル検索結果でもバーの下に表示
- レーダーチャートは全スキル（現状42種）を一度に表示すると軸が密集して読めなくなるため、「分野」タブ（サブカテゴリ単位。サブカテゴリが無いスキルはカテゴリ単位でグルーピング）で絞り込んで表示する。分野の選択はメンバー切り替え・評価時期切り替えをまたいで保持される。デフォルトは分野一覧の先頭（skillList内の出現順で決まる）
