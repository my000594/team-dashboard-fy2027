...
- マトリクス（スキル・保有資格どちらも共通の`.matrix-tbl`）は`table-layout:fixed`＋メンバー列を均等固定幅にしている（2026-08-12修正）。以前はブラウザの自動列幅計算に任せていたため、備考が長い人（例：谷野亘の「プロセス制御用言語（日立）」等）の列だけ異様に広がり列がガタガタになる不具合があった
...
```
氏名,提出日,ファイルリンク
米花 雅史,2026-08-05,https://syshd01-my.sharepoint.com/xxx
```
...
GITより出向、第1-1ラインに参画,2026-08-01,参画,劉 華雲,,確定
