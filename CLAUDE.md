...
- **`![説明](URL)`のURL部分がNotion側で実際にハイパーリンク化される（自動リンク化・手動でリンク設定）と、`sync-notion.mjs`の`getRichText`がhrefを`[表示](URL)`形式に復元してから抽出するため、結果的に`![表示](URL)`として正しく残る**（2026-08-23対応）。以前は`getRichText`がplain_textしか見ておらずhrefを無視していたため、リンク化された箇所はURLが消えて表示テキストだけが残る不具合があった（本文中の他のリンクや通常のURLでも起こり得る潜在バグだったが、画像記法の追加で表面化して発覚・修正）
