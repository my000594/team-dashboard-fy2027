...
```
- Notion側の出典：DBではなく「👥 組織構成」ページ本文の「組織構成図」セクション配下に貼られたファイル添付ブロック（外部ドライブ・SharePoint等へのリンク）。ページ内の掲載順（＝Notion上で追加した順）のままCSVに出力される。「組織構成図」セクションはトグルブロック（またはトグル化見出し）で、その子ブロックとしてファイルを入れる運用（見出し＋後続の兄弟ブロックという非トグル形式にも後方互換あり）。sync-notion.mjsの`org_chart.csv`セクションが両方に対応している
- 表示名：Notion側でファイルに設定した**キャプション**をそのまま使用（例：`2026年8月`）。キャプション未設定の場合は「組織構成図」とフォールバック表示
- 運用：月1回、その時点の組織構成図ファイルをNotionの「組織構成図」トグル配下に追加し、キャプションに対象年月を入力する。最大でも年12件（2026/8〜2027/7）の想定のため、ダッシュボード側は履歴アーカイブを持たず全件をmember.html（メンバー一覧の下）にチップ表示するだけ
...
- 対象ページ（DBクエリではなくページ本文のブロックを直接取得。`scripts/sync-notion.mjs`内`PAGE`定数）：
  - orgChart: 👥 組織構成ページ（「組織構成図」トグル配下のファイル添付 → org_chart.csv）
