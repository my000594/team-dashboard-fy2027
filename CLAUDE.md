...
### skill.html（スキルマップ）
- `data/skill.csv`（氏名×スキル名の縦持ち）と`data/members/member_master.csv`を並行fetchしてPapaParseで解析
...
- レーダーチャートは全スキル（現状49種）を一度に表示すると軸が密集して読めなくなるため、「分野」タブ（サブカテゴリ単位。サブカテゴリが無いスキルはカテゴリ単位でグルーピング）で絞り込んで表示する。分野の選択はメンバー切り替え・評価時期切り替えをまたいで保持される。デフォルトは分野一覧の先頭（skillList内の出現順で決まる）
...
- 備考：スキルの補足事項を書く自由記述欄。2026-07-28時点で資格系スキル（AWS認定・基本情報技術者等）はスキル名一覧から廃止されており、資格情報を書く運用は現状ない
