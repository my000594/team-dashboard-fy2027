...
└── data/
    ...
    ├── skill.csv           スキル評価（Notionエクスポート・縦持ち）
    └── members/
...
### skill.html（スキルマップ）
- `data/skill.csv`（氏名×スキル名の縦持ち）をfetchしてPapaParseで解析
- タブ3種：マトリクス（メンバー×スキルの一覧、レベルをドットで表示）／レーダーチャート（メンバーを選んでスキルレーダー表示）／スキル検索（スキル名で検索し保有者をレベル順表示）
- 上記3タブとは別に「評価時期」タブ（2026年8月／11月・2027年2月／5月）があり、3タブすべてに共通で適用される。選択中の時期はメンバー選択・タブ切り替えをまたいで保持される。デフォルトは最新時期。新しい時期が追加されたらskill.html内の`PERIODS`定数とscripts/sync-notion.mjsの`SKILL_PERIODS`定数の両方に追記し、このセクションも更新すること
- メンバー一覧・スキル一覧はいずれもCSV内の出現順をそのまま採用（member_master.csvとの突合はしていない）
...
米花雅史_Java,米花 雅史,Java,技術,プログラミング言語,4,,,,,2026-08-01
伊藤良多_Python,伊藤 良多,Python,技術,プログラミング言語,3,,,,,2026-08-01
```
...
- 氏名・スキル名・カテゴリ・サブカテゴリはいずれも自由記述可（Notionのセレクト値をそのまま採用）。カテゴリの色分けはknowledge.csvと同様、実データから動的に生成される
...
- 備考：AWS認定など1スキルで複数の資格・詳細を持つ場合に内訳を書く自由記述欄（例：「SAA, SOA保有」）。skill.htmlではマトリクスのドットのホバー（title属性）とスキル検索結果に表示される
- 1人あたりの行数は42項目固定ではない。「その他（自由記入）」に具体的なスキル名を追記する形で人によって行数が増減してよい（skill.html・sync-notion.mjsは行数を前提にしたロジックを持たない）
- Notion側データベース名：「🎯 スキル評価」（「2027年7月期」ページ配下、他のDBと同階層）
