...
米花雅史_AWS-CLF,米花 雅史,AWS-CLF,2023-01-15,2026-01-15,
米花雅史_基本情報技術者,米花 雅史,基本情報技術者,,,
```
- 1行＝1人×1保有資格（スキルのようなレベル評価ではなく、資格は「持っている／持っていない」の二値。保有している資格の行のみ存在し、未保有の資格は行自体を作らない）
- 氏名・資格名はいずれも自由記述可（Notionのセレクト値をそのまま採用）。資格名の初期カタログ：基本情報技術者／応用情報技術者／情報処理安全確保支援士／AWS-CLF／AWS-SOA／AWS-SAA／AWS-SAP／AWS-DVA／AZ-900／AZ-104／GCP-ACE／その他（自由記入）。Notion側で自由に選択肢を追加してよく、コード側は追随不要
- 取得日：資格を取得した年月日
- 有効期限：更新が必要な資格（AWS・Azure・GCP認定など）のみ設定。基本情報技術者・応用情報技術者・情報処理安全確保支援士のような無期限資格は空欄のままにする
- 備考：自由記述の補足欄
- skill.htmlのマトリクスタブ内に「🎓 保有資格」カードとして、資格名×メンバーのマトリクス表示される。有効期限のあるものは今日の日付基準で「期限切れ」（赤）／「更新目安＝期限まで90日以内」（amber）／それ以外（緑）に自動判定して表示。取得日は年月表記でセル下に表示
