...
- **「チーム全体」レーダー**（2026-08-18追加）：メンバーチップの先頭に擬似メンバー`👥 チーム全体（○名平均）`（内部キー`TEAM_KEY`）を追加。選択すると、表示中の軸（総合タブ＝分野平均、個別分野タブ＝スキルレベル）ごとに全メンバーの平均・最小・最大を算出し、①最小〜最大の帯（塗りつぶし。Chart.jsの2データセット＋`fill:'-1'`で表現）②チーム平均の実線③（任意）本人比較の点線、を重ねて表示する。③は`localStorage`の`you_widget_member`（you-widget.jsが使うキーをそのまま流用）にブラウザ記憶があるメンバーのみ自動で重なる。前回評価時期との比較（上記）はチーム表示では対象外（個人選択時のみ）。この擬似メンバーはレーダーチャートタブ専用で、マトリクスタブのモバイル一覧（`mobileMatrixChips`）には出さない（チーム集計の一覧表示は未実装のため）。`selectMember()`・`buildMemberChips()`はこの擬似メンバーかどうかで分岐しており、チーム選択時はモバイルのスキル/資格一覧・マトリクス側の選択状態には影響しない
