...
| ライン名・カラーを変えた | `config/org.json`のlines配列・lineLabelMap（詳細は「組織設定」節参照） |
...
├── CLAUDE.md           このファイル
├── config/
│   └── org.json        組織固有設定（ライン名・管理者名・カラー・参考メンバー等。詳細は「組織設定」節参照）
└── data/
...
└── docs/
    ...
    └── PATTERNS.md          パターン集（他部署・他社へ展開する際の「サンプル実装ページの型」。career/sales/reports/meetings/chronicleが対象。詳細は当該節参照）
```

---

## 組織設定（config/org.json）

このダッシュボードを他部署・他社に渡す際の「組織固有値の差し替え口」として2026-09-11に導入。ライン名・管理者名・カラー・所属ラインの表記ゆれ変換・参考メンバー・スキル評価時期など、渡す相手ごとに変わる値をこのファイル1つに集約している。渡す際はこのファイルの中身を書き換えるだけでよい（コード本体には手を入れない）。

```json
{
  "orgLabel": "SI部 第1ライン",
  "fyLabel": "FY2027 Dashboard",
  "lines": [
    { "key": "第1ライン",   "manager": "米花 雅史", "color": "#3b82f6", "bg": "rgba(59,130,246,0.12)" },
    { "key": "第1-1ライン", "manager": "伊藤 良多", "color": "#22c55e", "bg": "rgba(34,197,94,0.12)"  }
  ],
  "lineLabelMap": { "百瀬 陽一Bライン": "第1ライン", "米花雅史Ｋライン": "第1-1ライン" },
  "referenceMembers": ["百瀬 陽一"],
  "skillPeriods": ["2026年8月", "2026年11月", "2027年2月", "2027年5月"]
}
```

- 参照元：`nav.js`（`orgLabel`/`fyLabel`をサイドバーのロゴ表記に使用）、`member.html`（`lines`→`LINE_ORDER`/`LINE_META`/`LINE_COLORS`、`lineLabelMap`→`LINE_LABEL_MAP`、`referenceMembers`→`REFERENCE_MEMBERS`）、`skill.html`（`skillPeriods`→`PERIODS`、`referenceMembers`→`REFERENCE_MEMBERS`）、`index.html`（`referenceMembers`→課員数カウントから参考メンバーを除外する判定）。各ページの`loadOrgConfig()`（またはindex.htmlは`Promise.all`内の直接fetch）が`config/org.json`を取得し、CSV取得と同じタイミングで該当する変数に反映する
- **取得に失敗しても現行運用と同じ値にフォールバックする**（各ページの`applyOrgConfig()`内にデフォルト値としてハードコード済み）。`config/org.json`自体を配布物から削除してしまっても表示は壊れない設計
- ライン・管理者・カラーを変える場合はこのファイルの`lines`配列を編集するだけでよい。`member.html`側の定数を直接触る必要はなくなった（2026-09-11、パッケージ配布を見据えた改修）
- `skillPeriods`はスキル評価時期の一次情報源。ただし`scripts/sync-notion.mjs`の`SKILL_PERIODS`・`you-widget.js`の`YOU_SKILL_PERIODS`は独立した定数のままなので、新しい時期を追加する際はこの3箇所（`config/org.json`・`scripts/sync-notion.mjs`・`you-widget.js`）を揃えて更新すること
- career.html（等級・昇格基準）・meetings.html（方針文）・sales.html・reports.html・chronicle.htmlは、御社固有の制度・実データに強く依存するためconfig化していない。他組織に渡す場合はコードごと「参考実装」として渡し、その組織の実情に合わせて作り直してもらう方針（詳細は今後の課題・展開方針を参照）
...
LINE_COLORS・LINE_META・LINE_LABEL_MAP・REFERENCE_MEMBERSは、2026-09-11以降`config/org.json`から読み込む（member.html内では`let`宣言のみで、実体は`init()`冒頭の`applyOrgConfig()`が設定する。詳細は「組織設定（config/org.json）」節を参照）。ラインが変わった場合、または所属ラインの生値の表記が変わった場合は`config/org.json`を更新すればよく、member.html本体には手を入れる必要がない。
...
- **参考メンバーのピンク表示**（2026-08-18追加、同日中に判定方法を変更）：部長・百瀬陽一が自分のスキル・資格を参考値として登録したケースを、マトリクス・保有資格表の列見出し（ピンク背景＋🍑アイコン、`isRefMember()`/`memberThHTML()`）と列全体（薄いピンクの縦帯＋左境界線、`memberTdOpen()`）、レーダーチャート／モバイル一覧のメンバーチップ（`.ref-chip`、🍑アイコン付き）、「重ねて比較」セレクトの選択肢で見分けられるようにする。「ライン外・参考値」等の説明文言は付けない（アイコン＋色だけで十分という判断）。ピンクの色トークン（`--pink`/`--pink-dim`）はstyle.cssに追加済み
  - **対象は`teamRosterMembers`との不一致による自動判定ではなく、`REFERENCE_MEMBERS`（明示的な氏名リスト、現状`['百瀬 陽一']`）で管理する**。当初は在籍名簿に無い人を自動でピンクにする実装にしていたが、「新たに配属される社員」が名簿（member_master.csv）登録前の一時的な期間だけmembers末尾に混ざるケース（表記ゆれ救済フォールバックと同じ仕組み）まで誤って参考データ扱いになってしまうため、明示リスト方式に変更した。今後また参考値のみのメンバーが出た場合は、その都度`config/org.json`の`referenceMembers`に氏名を追記する運用（自動追随はしない。2026-09-11以降、実体はmember.html・skill.html双方とも`config/org.json`から読み込む）
...
- デザインのさらなる洗練
- **他部署・他社への展開準備（2026-09-11着手）**：`config/org.json`によるコア部分（member/skill/knowledge/info＋共通基盤）の組織非依存化と、career/sales/reports/meetings/chronicleの「パターン集」化（`docs/PATTERNS.md`）まで完了。残タスク：①渡す際のgit履歴サニタイズ手順の確立（過去コミットに実メンバーの個人情報が残っているため、`git clone`ではなく現時点のファイルのみで新規履歴を作る必要がある。詳細はルートCLAUDE.mdの「組織設定」節および検討の経緯を参照）、②Notion側の複製可能テンプレート（空のDB一式）の用意、③セットアップ手順のAI主導スキル化（`.claude/skills/setup.md`等）、④簡易利用条件（ライセンス）文言の作成
