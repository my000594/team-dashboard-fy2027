...
- **資格分野が「研修」の行は保有資格として扱わない**（2026-08-10追加）。資格そのものではなく研修受講の記録をこのDBで管理したいケース向けの除外で、skill.html（マトリクス・区分別/分野別タブ・モバイル一覧）・index.htmlの「あなたの状況」ウィジェットの保有資格件数のどちらからも除外される（取得元のCSVレベルではなくフロント側のフィルタで除外しているため、Notion側でこの行自体を消す必要はない）
