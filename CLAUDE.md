...
- デジタルバッジ：Notionの「デジタルバッジ」ファイル添付から抽出したURL（複数ある場合はカンマ区切り、skill.htmlは先頭の1件のみ使用）。Notion直アップロードのファイルは期限付きプリサインURLになるため、リンク切れの可能性がある点はmeeting_plan.csvの資料リンクと同じ
- 備考：自由記述の補足欄
